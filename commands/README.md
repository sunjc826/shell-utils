...
2. They can be invoked directly because bash-utils/lib/commands has been added to PATH (assuming bu-entrypoint has been sourced). `bu-new-command.sh ...`
3. Even without having sourced `bu_entrypoint.sh`, a script can be executed directly! E.g. `/path/to/commands/bu-new-command.sh ...`. E.g. on my machine: `/home/$USER/Documents/bash-utils/commands/bu-new-command.sh --help`.
...
2. They can be sourced without the full path because bash-utils/lib/commands has been added to PATH (assuming bu-entrypoint has been sourced). `source bu-import-environment.sh ...`
