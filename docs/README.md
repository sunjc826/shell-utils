...
- Everything is a script: Even the "built-in" [commands][commands] are just scripts. In fact, [bu-import-environment.sh][bu-import-environment] and [bu-get-command.sh](../commands/bu-get-command.sh) are both generated by [bu-new-command.sh](../commands/bu-new-command.sh) (which is itself a script) from a template, then filled in. Thus shell-utils aims for extensibility by avoiding primitives that lie outside the framework's scripting system.
...
### Library
The core library is in [core](../lib/core/)
- [bu_core_base.sh](../lib/core/bu_core_base.sh) provides the core library functions for the framework
- [bu_core_autocomplete.sh](../lib/core/bu_core_tmux.sh) provides the autocompletion functionality for the `bu` command and the scripts it invokes.
...
- [bu_core_tmux.sh](../lib/core/bu_core_tmux.sh) provides imperative utilities (`bu_spawn`) for orchestrating jobs across tmux panes.
...
[commands]: {{ github_base }}/commands
[bu-import-environment]: {{ github_base }}/commands/bu-import-environment.sh
[bu_user_defined_decl]: {{ github_base }}/bu_user_defined_decl.sh
