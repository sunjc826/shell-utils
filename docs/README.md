...
Bash-utils is a scripting framework for Bash. 
...
- Everything is a script: Even the "built-in" [commands][commands] are just scripts. In fact, [bu-import-environment.sh][bu-import-environment] and [bu-get-command.sh][bu-get-command] are both generated by [bu-new-command.sh][bu-new-command] (which is itself a script) from a template, then filled in. Thus bash-utils aims for extensibility by avoiding primitives that lie outside the framework's scripting system.
...
The `BU_USER_DEFINED_` prefix indicates variables that are not defined anywhere within bash-utils. It is expected that the user defines them externally.

To get the bash-utils library into one file, run
```sh
