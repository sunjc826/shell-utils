...
- Everything is a script: Even the "built-in" [commands](./commands/) are just scripts. In fact, [bu-import-environment.sh](./commands/bu-import-environment.sh) and [bu-get-command.sh](./commands/bu-get-command.sh) are both generated by [bu-new-command.sh](./commands/bu-new-command.sh) (which is itself a script) from a template, then filled in. Thus shell-utils aims for extensibility by avoiding primitives that lie outside the framework's scripting system.
