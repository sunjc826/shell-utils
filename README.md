...
- Everything is a script: Even the "built-in" [commands](./commands/) are just scripts. In fact, [bu-import-environment.sh](./commands/bu-import-environment.sh) and [bu-get-command.sh](./commands/bu-get-command.sh) are both generated by [bu-new-command.sh](./commands/bu-new-command.sh) from a template, then filled in. Thus shell-utils aims for extensibility by avoiding core commands that lie outside the script framework's scripting system.
